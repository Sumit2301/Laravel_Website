			...
			<table class="table table-bordered table-striped table-hover ajaxTable datatable datatable-Employee" id="spTable">
				<thead>
					<tr>
						...
						<th>College</th>
						<th class="text-center" style="width: 100px;">Brochure</th>
				...
				<tbody>
					@foreach($downloadbrochures as $key=>$row)
					<tr>
						...
						<td class="text-center">
							@if($row->brochure)
							<a class="btn btn-dark btn-sm" href="{{ asset('uploads/college-brochure/'.$row->brochure) }}" target="_blank">View Brochure</a>
							@endif
						</td>
						<td class="text-center">{!! ($row->status==1)?"<b class='btn btn-success btn-sm'>Open</b>":"<b class='btn btn-danger btn-sm'>Closed</b>" !!}<!-- {!! ($row->status==1)?"<a href='".url("admin/downloadbrochures/updateStatus/".$row->id."/2")."' class='btn btn-success btn-sm'>Open</a>":"<a  href='".url("admin/downloadbrochures/updateStatus/".$row->id."/1")."' class='btn btn-danger btn-sm'>Closed</a>" !!} --></td>
...
<script>
  ...
  submitHandler: function(form) {
   ...
   var rechargeform = document.getElementById('edit-form');
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo url('admin/downloadbrochures/remark_process'); ?>",
        data: fd,
        contentType: false,           
        processData:false, 
