			...
			<form action="{{ route("admin.cities.store") }}" method="POST" enctype="multipart/form-data">
				<div class="row">
					...
					<div class="col-md-6 form-group {{ $errors->has('city_code') ? 'has-error' : '' }}" style="margin-top: 20px;">
						...
						<input type="number" id="city_code" name="city_code" class="form-control" value="{{ old('city_code') }}" required >
				  ...
				  <div class="form-group col-md-6">
					...
					<select class="form-control" name="state_code" id="state_code">
						...
						<option value="{{ $state->id }}" @if(old('state_code')==$state->id) selected @endif >{{ $state->state_name }}</option>
						@endforeach
					</select>
				  </div>
