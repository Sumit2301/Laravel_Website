			...
			<table class="table table-bordered table-striped table-hover ajaxTable datatable datatable-Employee" id="spTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $row->subject }}</td>
						<td>{{ $row->message }}</td>
						<td class="text-center">{!! ($row->status==1)?"<b class='btn btn-success btn-sm'>Open</b>":"<b class='btn btn-danger btn-sm'>Closed</b>" !!}<!-- {!! ($row->status==1)?"<a href='".url("admin/contacts/updateStatus/".$row->id."/2")."' class='btn btn-success btn-sm'>Open</a>":"<a  href='".url("admin/contacts/updateStatus/".$row->id."/1")."' class='btn btn-danger btn-sm'>Closed</a>" !!} --></td>
...
<script>
  ...
  submitHandler: function(form) {
   ...
   var rechargeform = document.getElementById('edit-form');
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo url('admin/contacts/remark_process'); ?>",
        data: fd,
        contentType: false,           
        processData:false, 
