...
<div class="card col-md-10 p-0">
    ...
    <div class="card-body">
        <form action="{{ url("admin/colleges/update",$id) }}" method="POST" enctype="multipart/form-data">
		<div class="row">
...
<script type="text/javascript">
	...
	function selectCity(stid){		   
	   $.ajax({
		   type:'POST',
		   url:'{{ url("/cities") }}',
		   data:{'_token': '<?php echo csrf_token() ?>','stateid':stid,'slct':{{$college->city}}},
...
var courseslist = <?=json_encode($courselist)?>;
var ranktypelist = <?=json_encode($ranktypelist)?>;
var updated_fields = <?=json_encode($college_course_detail)?>;
