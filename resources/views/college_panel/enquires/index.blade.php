		...
		<div class="card-body table-responsive">
			<table class="table table-bordered table-striped table-hover ajaxTable datatable datatable-Employee" id="spTable">
				...
				<tbody>
					...
					@foreach($college_enquires as $key=>$row)
					<tr>
						<td class="text-center">{{ $key+1 }}</td>
						<td>{{ $row->name }}</td>
						<td>{{ $row->mobile }}</td>
						<td>{{ $row->state }}</td>
						<td>{{ $row->neet_score }}</td>
						<td>{{ $row->landingpage }}</td>
						<td class="text-center">{!! ($row->status==1)?"<b class='btn btn-success btn-sm'>Open</b>":"<b class='btn btn-danger btn-sm'>Closed</b>" !!}</td>
						<td><div style="width: 80px;">{{ $row->remark_date }}</div></td>
						<td>{{ $row->remark }}</td>
						<td class="text-center">{{ $row->created_at }}</td>
					<td class="text-center"><button type="button" class="btn btn-primary btn-sm" onclick="editStatus({{ $row->id }},{{ $row->status }}, '{{ $row->remark_date }}', '{{ $row->remark }}')"><i class="fa fa-edit"></i></button></td>
					</tr>
					@endforeach
				</tbody>
			</table>
...
<script>
  ...
  submitHandler: function(form) {
   ...
   var rechargeform = document.getElementById('edit-form');
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo url('admin/college-enquires/remark_process'); ?>",
        data: fd,
        contentType: false,           
        processData:false, 
